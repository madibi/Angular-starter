...
import {UserState} from "@web-commons/store/user/user.state";
import {ActivatedRoute, Router} from "@angular/router";
import * as USER_ACTIONS from '@web-commons/store/user/user.action';
import {RequestCodeType} from "@web-commons/schema/user/enum/request-code-type.enum";
import {RequestCodeSmsType} from "@web-commons/schema/user/enum/request-code-sms-type.enum";
import {configuration} from '@web-commons/settings/configuration';
...
export class RequestCodeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      this.smsType = params['sms-type'].toLowerCase() === 'firebase' ? RequestCodeSmsType.FIREBASE : RequestCodeSmsType.APP;
    });
  }
}
